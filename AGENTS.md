...
- `test-backend-ops` - full sweep on CPU + CUDA0 (23k+ cases on the RTX 5090 dev box); now rejects 0/0 as FAIL
...
- `test-backend-ops` reports `Backend ...: OK` even when every case was skipped: the backend verdict is `n_ok == tests_run`, and 0/0 passes. Fixed in `test-backend-ops.cpp`: now checks `tests_run > 0` before returning OK. See issue #242 (fixed).
